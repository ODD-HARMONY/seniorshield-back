...
유튜브 쇼츠 URL 하나를 받아 **AI 생성 여부**, **허위정보 여부**, **광고·사기 의심 여부**, **커뮤니티 의심 신호**를 반환합니다.
...
5개 항목이 모두 `ok`이면 정상입니다.  
(`extractor`·`analyzer`·`factcheck`는 모두 동일한 `ss-analyzer` 컨테이너를 가리킵니다.)
...
| `stages.ad_verify` | 광고 검증 스테이지 트리거 시에만 포함 |
...
| `POST` | `/api/ad` | `{"frames_base64": ["..."], "subtitle_text": "...", "initial_label": "likely_false_ad", "lang": "ko"}` |
...
| `MODEL_IMAGE` | `gemini-2.0-flash` | AI 이미지 감지 및 광고 검증 모델 |
...
docker compose logs -f analyzer   # Gemini API 호출 / yt-dlp / 팩트체크 로그
```
...
| 증상 | 원인 | 조치 |
|------|------|------|
| `extractor_error` 오류 | YouTube 봇 차단 (429) | `cookies.txt` 생성 후 `docker compose restart analyzer` |
| `video_unavailable` 오류 | 영상 삭제/비공개/지역 제한 | 다른 URL로 테스트 |
| `video_too_long` 오류 | `MAX_DURATION_SEC`(180초) 초과 URL | 짧은 영상 URL 사용 |
| `analyzer` 502 오류 | Gemini API 키 오류 또는 모델 미지원 | `.env`의 `GEMINI_API_KEY`, `MODEL_*` 확인 |
| `image` 단계 타임아웃 | Gemini 응답 지연 | `ANALYZER_TIMEOUT_MS` 값 증가 |
...
| HTTPS 504 오류 | nginx `proxy_read_timeout` 초과 | `nginx/nginx.conf`의 `proxy_read_timeout` 값 증가 |
| HTTPS 접속 안 됨 | nginx가 실행되지 않음 | `docker compose --profile ssl up -d` 로 실행했는지 확인 |
| SSL 인증서 오류 | 인증서 만료 또는 경로 불일치 | `nginx/certs/` 재복사 후 `docker compose --profile ssl restart nginx` |
| nginx 재시작 후에도 502 | 컨테이너 재구성 후 nginx IP 캐시 | `docker compose --profile ssl restart nginx` 로 재시작 |
