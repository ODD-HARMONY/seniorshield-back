...
- **자막**: `yt-dlp`로 VTT 형식 다운로드 → `webvtt-py`로 파싱, HTML 태그 제거, 중복 세그먼트 병합
...
프롬프트 파일(`prompts/*.txt`)은 컨테이너 볼륨으로 마운트되어 **재빌드 없이** 교체 가능합니다.
...
- 한국어 주장은 ko→en 번역 후 재시도 (fallback)
...
| 테이블 | 용도 |
|--------|------|
| `analysis_cache` | 분석 결과 캐싱 (URL 해시 + 모델 버전으로 키 구성) |
| `suspicion_reports` | 개별 의심 신고 로그 (1인 1표 보장: `UNIQUE KEY(url, client_id)`) |
| `suspicion_aggregate` | 영상별 의심 카운트 집계 (빠른 조회용) |
...
POST /api/analyze
  ...
  URL 정규화 (UrlValidator.normalize)
        │
        ▼
  캐시 조회 (url_hash + 모델 버전 일치 시 즉시 반환)
        │ miss
        ▼
  [1] extract ──────────────────────────────────────── 순차
  ...
  [2] classify ─────────────────────────────────────── 순차
        ...
        ▼
  [3] image ◄──────────────────┐
  [4] info  ◄──── 병렬 ────────┤  (CompletableFuture)
        │         (informational=false이면 info 스킵)
        ▼
  [5] factcheck (info.claims 순차 호출)
        │
        ▼
  ResultAggregator.aggregate()
        │
        ▼
  SuspicionDao.getCount(url)
        │
        ▼
  ResultAggregator.applySuspicionWeight()
        │
        ▼
  DB 캐시 저장
        │
...
```
캐시 키 = SHA-256(canonical_url) + MODEL_CLASSIFY + MODEL_INFO + MODEL_IMAGE
```
...
│       ├── servlet/            # 7개 서블릿
│       └── util/               # UrlValidator, HashUtil, JsonUtil
│
